...
void input_logic(vector<Node> &nodes2,int operand_num, int input1_type, int input1_id, int input2_type, int input2_id,
            ...
            } else    //有两个操作数，一个输入到sa_direct,一个写入sa
            {
                ...
                if ((input1_type != 6 || input1_id != decide_array_id) &&
                    ...
                    if (idea_dse == 0) {
                        ...
                        if (now->depend1!= nullptr) //depend1不是立即数,则是一个=节点, 写入sa阵列中
                        {
                            if (cap_array_lost(nodes2,2, decide_array_id, array_list1, array_list2, array_list3) == 0) {
                                write_cover(nodes2,0, now->depend1, 2, decide_array_id,1, array_list1, array_list2, array_list3);
                            }
                            else
                            {
                                //更新阵列的存储节点表
                    ...
                    else//2.depend2写入
                    {
                        ...
                        {
                            if (cap_array_lost(nodes2,2, decide_array_id, array_list1, array_list2, array_list3) == 0) {
                                write_cover(nodes2,0, now->depend2, 2, decide_array_id,1, array_list1, array_list2, array_list3);
                            }
                            else
                            {
                                //更新阵列的存储节点表
...
void output_logic(vector<Node> &nodes2,int decide_array_type, int decide_array_id, int op_type, Node *now, \
    ...
    switch (decide_array_type) {
        case 1: //LUT
        {
            ...
            op_lut(nodes2,op_type, decide_array_id, now, time_n, array_list1, array_list2, array_list3);
            //需要一个函数判断是否需要调用模块
        ...
        case 2: {
            op_sa(nodes2,op_type, decide_array_id, now, time_n, array_list1, array_list2, array_list3);
            return;
        }

        case 3: {
            op_magic(nodes2,op_type, decide_array_id, now, time_n, array_list1, array_list2, array_list3);
            return;
        }
...
void op_lut(vector<Node> &nodes2,int op_type, int decide_array_id, Node *now, double time_now, vector<lut_arr> &array_list1,
    ...
    if (array_list1[decide_array_id].lut_out > 0)  //节点id>=1
    {
        ...
        {
            if (find_node_by_number(nodes2,lut_out_now)->out_degree > 0 &&
                wb_empty(find_node_by_number(nodes2,lut_out_now))) {
                //设置优先级，buffer只能写回本阵列
                write_back(nodes2,1, find_node_by_number(nodes2,lut_out_now)->finish_id, find_node_by_number(nodes2,lut_out_now),
    ...
    if (array_list2[decide_array_id].sa_out > 0)  //节点id>=1
    {
        ...
        else //否则判断一下其写回表和出度
        {
            if (find_node_by_number(nodes2,sa_out_now)->out_degree > 0 &&
                ...
                //设置优先级，buffer只能写回本阵列
                write_back(nodes2,2, find_node_by_number(nodes2,sa_out_now)->finish_id,find_node_by_number(nodes2,sa_out_now),
                           array_list1, array_list2, array_list3);
            }
        }
