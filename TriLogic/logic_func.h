...
//时间特性,更新阵列的开始时间，结束时间
double
time_now(vector<lut_arr> &array_list1, vector<sa_arr> &array_list2, vector<magic_arr> &array_list3, Nodes *node_now,
         int decide_array_type = -2, int decide_array_id = -2);

double time_only_lut(vector<lut_arr> &array_list1, vector<sa_arr> &array_list2, vector<magic_arr> &array_list3, Nodes *node_now,
                     int decide_array_type = -2, int decide_array_id = -2);
double time_only_sa(vector<lut_arr> &array_list1, vector<sa_arr> &array_list2, vector<magic_arr> &array_list3, Nodes *node_now,
                    int decide_array_type = -2, int decide_array_id = -2);
double time_only_magic(vector<lut_arr> &array_list1, vector<sa_arr> &array_list2, vector<magic_arr> &array_list3, Nodes *node_now,
                       int decide_array_type = -2, int decide_array_id = -2);

//update the time of do_array
void time_update(int op_type, int do_type, int array_id, double time_now, Nodes *node_now,
...
void data_read_sa(int No_depend, int &input_type, int &input_id, int decide_array_type, int decide_array_id, Nodes *now,
                  vector<lut_arr> &array_list1, vector<sa_arr> &array_list2, vector<magic_arr> &array_list3);
void data_read_magic(int No_depend, int &input_type, int &input_id, int decide_array_type, int decide_array_id, Nodes *now,
                     vector<lut_arr> &array_list1, vector<sa_arr> &array_list2, vector<magic_arr> &array_list3);
...
/*中层：阵列行为*/
void find_input(int &array_type, int &array_id, Nodes *node_depend = nullptr);//寻找操作数来源,node_depend指向nodes中的节点
...
input_logic(int operand_num, int input1_type, int input1_id, int input2_type, int input2_id, int decide_array_type,
            int decide_array_id,
            Nodes *now, vector<lut_arr> &array_list1, vector<sa_arr> &array_list2, vector<magic_arr> &array_list3);
...
//void erase_array(Nodes *now, vector<lut_arr> &array_list1, vector<sa_arr> &array_list2, vector<magic_arr> &array_list3);
...
void place_num(vector<vector<int>> data_exchange, int **data_transfer, int array_id, int list1_size, int list2_size);
