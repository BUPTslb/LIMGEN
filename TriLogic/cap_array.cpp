...
int cap_array_lost(int decide_array_type, int decide_array_id, vector<lut_arr> &array_list1, \
    ...
    int cap_array;
    if (decide_array_type==2)   cap_array=array_list2[decide_array_id].row_num;
    if (decide_array_type==3)   cap_array=array_list3[decide_array_id].row_num;
    switch (decide_array_type) {
        ...
        case 2://SA
        {
            ...
            //检验越界
            //cout<<"sa row num "<<array_list2[decide_array_id].row_num<<endl;
        ...
        case 3://MAGIC
        {
            ...
            //cout<<"magic row num"<<array_list3[decide_array_id].row_num<<endl;
    ...
    switch (decide_array_type) {
        ...
        case 2://SA，只计算出度为0的
        {
            ...
            for (int i=0;i<array_list2[decide_array_id].store_node.size();i++) {
                if (find_node_by_number(array_list2[decide_array_id].store_node[i]))  //可以找到这个节点
                {
                    if (find_node_by_number( array_list2[decide_array_id].store_node[i])->out_degree == 0) //节点出度为0，不再被需要
                    {
                        cap_array++;
                    ...
                    {
                        //但是在多个地方存储的都有
                        if (num_node_position(find_node_by_number( array_list2[decide_array_id].store_node[i]))>1) //在多个地方存储
                        {
                            cap_array++;
                        }
        ...
        case 3://MAGIC
        {
            ...
            for (int i=0;i<array_list3[decide_array_id].store_node.size();i++) {
                if (find_node_by_number(array_list3[decide_array_id].store_node[i])) //能找到该节点
                {
                    if (find_node_by_number( array_list3[decide_array_id].store_node[i])->out_degree ==0) //节点出度为0，不再被需要
                    {
                        cap_array++;
                    }
                    else //出度>0
                    {
                        if (num_node_position(find_node_by_number(array_list3[decide_array_id].store_node[i]))>1) //在多个地方存储
                        {
                            cap_array++;
                        }
               ...
               vector<sa_arr> &array_list2, vector<magic_arr> &array_list3)
{
    ...
    switch (decide_array_type) {
        case 2://sa-out
            ...
            for (int i=0;i<array_list2[decide_array_id].store_node.size();i++) {
                if (find_node_by_number(array_list2[decide_array_id].store_node[i])) //能找到该节点
                    capped_now++;
            }
            break;
        case 5://sa-buffer
