                ...
                //不一定有依赖，如果常用的立即数，会将其交给寄存器
            {
                ...
                switch (controlstep2[i][j]->depend1->do_type) {
                    case -1:  //来自寄存器，不知到写回到哪里,直接赋值应该不用写的
                    {
                        ...
                        write_back(-1, -1, controlstep2[i][j], array_list1, array_list2, array_list3, back_type,
                                   back_id);
                    }
                        break;
                    case 1: //来自lut的执行结果
        ...
        if (No_depend == 1) //depend1
        {
            ...
            if (input_type == 1) //来自lut的输出，写回表肯定是空的
            {
                    ...
                    && array_list1[input_id].lut_out != now->depend1->node_id)//对不上
                {
                    ...
                    write_back(1, input_id, now->depend1, array_list1, array_list2, array_list3, back_type, back_id);
                    //写回到哪里从哪里接着读取
                    //重新调用find_input
        ...
        if (No_depend == 2) //depend2
        {
            ...
            if (input_type == 1) //来自lut的输出,写回表肯定是空的
            {
                    ...
                    && array_list1[input_id].lut_out != now->depend2->node_id)//对不上
                {
                    ...
                    write_back(1, input_id, now->depend2, array_list1, array_list2, array_list3, back_type, back_id);
