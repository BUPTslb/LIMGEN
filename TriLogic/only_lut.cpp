...
void only_lut(vector<vector<Node>> controlstep) {
        ...
        for (int j = 0; j < controlstep2[i].size(); ++j) {
                ...
                //不一定有依赖，如果常用的立即数，会将其交给寄存器
            {
                ...
                if (controlstep2[i][j]->depend1 == nullptr) {
                    ...
                    double time = time_only_lut(nodes2,array_list1, array_list2, array_list3, controlstep2[i][j], -1, -1);
                ...
                double time_n = time_only_lut(nodes2,array_list1, array_list2, array_list3, controlstep2[i][j]);//开始执行当前节点的时间
                //如果默认其为直接输出的out,则不会对其他产生任何影响，不需要进行time_update
                //c=a op b || c=a op -1 || c = a
                cout<<"等号依赖节点的do_type: "<<controlstep2[i][j]->depend1->do_type<<endl;
                    ...
                    case -1:  //来自寄存器，不知到写回到哪里,直接赋值应该不用写的
                    {
                        ...
                        if (back_type == 3) {
                            if (array_list3.empty())
                                back_id = build(nodes2,3, 0, array_list1, array_list2, array_list3);
                            else
                                back_id = array_list3[rand() % array_list3.size()].array_id;
                        }
                        cout<<"调用write_back_lut写回"<<back_type<<endl;
                        write_back_lut(nodes2,-1, -1, controlstep2[i][j], array_list1, array_list2, array_list3, back_type,
                                   back_id);
                    }
                        break;
                    case 1: //来自lut的执行结果
                    {
                        //规定：先假设其执行类型为lut-out,但是不更新时间能量，到再次使用时候再更新
                        ...
                        if (array_list1[controlstep2[i][j]->depend1->finish_id].lut_out > 0)  //节点id>=1
                        {
                            ...
                            cout<<"当前=阵列的lut-out的类型为："<<on_op<<endl;
                            if (on_op > 0) //不是等号，直接更新lut-out
                                array_list1[controlstep2[i][j]->depend1->finish_id].lut_out = controlstep2[i][j]->node_id;
                            else //否则判断一下其写回表和出度
                            {
                                cout<<"当前lut-out: "<<lut_out_now<<endl
                                    <<"当前=阵列的lut-out的节点的出度为："<<find_node_by_number(nodes2,lut_out_now)->out_degree<<endl
                                    <<"其节点写回表的大小为"<<num_node_position(nodes2,find_node_by_number(nodes2,lut_out_now))<<endl;
                                //不管其出度是否>0，都应该存起来？
//                                if (find_node_by_number(nodes2lut_out_now)->out_degree > 0 &&
//                                    wb_empty(find_node_by_number(nodes2lut_out_now)))
                                if (wb_empty(find_node_by_number(nodes2,lut_out_now)))
                                {
                                    cout << "调用write_back将lut_out写回" << endl;
                                    ...
                                    write_back_lut(nodes2,1, find_node_by_number(nodes2,lut_out_now)->finish_id,
                    ...
                    case 3: //操作数来自magic存储
                    {
                        ...
                        //也是A=A的类型
                        write_back_lut(nodes2,3, controlstep2[i][j]->depend1->finish_id, controlstep2[i][j], array_list1,
                    ...
                    case 4: //操作数来自lut-buffer
                    {
                        ...
                        //也是A=A的类型
                        write_back_lut(nodes2,4, controlstep2[i][j]->depend1->finish_id, controlstep2[i][j], array_list1,
                                   array_list2, array_list3, back_type, back_id);
            ...
            if (array_list1.empty())
                do_array_id = build(nodes2,1, type_operation, array_list1, array_list2, array_list3);
            ...
            {
                cout << "操作数个数为1" << endl;
                data_read_lut(nodes2,1, input1_type, input1_id, do_array_type, do_array_id, controlstep2[i][j],
            ...
            } else //有两个操作数
            {
                ...
                data_read_lut(nodes2,1, input1_type, input1_id, do_array_type, do_array_id, controlstep2[i][j],
                              array_list1, array_list2, array_list3);
                data_read_lut(nodes2,2, input2_type, input2_id, do_array_type, do_array_id, controlstep2[i][j],
                              array_list1, array_list2, array_list3);
            }
            ...
            //将数据输入到执行阵列：input逻辑
            input_logic_lut(nodes2,operand_num, input1_type, input1_id, input2_type, input2_id, do_array_type, do_array_id,
                            controlstep2[i][j],
                            array_list1, array_list2, array_list3);

            //执行运算,要更新finish_id
            output_logic_lut(nodes2,do_array_type, do_array_id, type_operation, controlstep2[i][j], array_list1, array_list2,
                             array_list3);
...
double time_only_lut(vector<Node> &nodes2,vector<lut_arr> &array_list1, vector<sa_arr> &array_list2,
    ...
    cout << "time_only_lut函数中的decide_array_type = " << decide_array_type << endl;
    ...
    cout << "time_only_lut函数中阵列的over_time: " << time_do_array << endl;
...
//写回 from_type表示写回的类型 0:Reg 1:lut-out 2:sa-out 3:magic存储 4:sa-buffer 5：lut-buffer 6:sa存储
                    ...
                    if (back_type == 4) back_id = node_now->depend1->finish_id;
                    write_back_lut(nodes2,1, node_now->depend1->finish_id, node_now->depend1, array_list1, array_list2,
                               array_list3,
                               back_type, back_id);

                    time_n = time_only_lut(nodes2,array_list1, array_list2, array_list3, node_now, decide_array_type,
...
//写回 from_type表示写回的类型 0:Reg 1:lut-out 2:sa-out 3:magic存储 4:lut-buffer 5：sa-buffer 6:sa存储
                ...
                if (back_type == 4) back_id = node_now->depend1->finish_id;
                write_back_lut(nodes2,1, node_now->depend1->finish_id, node_now->depend1, array_list1, array_list2, array_list3,
                           back_type, back_id);


                cout << "time_only_lut中的write_back没问题" << endl;
                time_n = time_only_lut(nodes2,array_list1, array_list2, array_list3, node_now, decide_array_type,
                                       decide_array_id);//更新时间
            }
...
//写回 from_type表示写回的类型 0:Reg 1:lut-out 2:sa-out 3:magic存储 4:sa-buffer 5：lut-buffer 6:sa存储
                ...
                if (back_type == 4) back_id = node_now->depend2->finish_id;
                write_back_lut(nodes2,1, node_now->depend2->finish_id, node_now->depend2, array_list1, array_list2, array_list3,
                           back_type, back_id);
...
void data_read_lut(vector<Node> &nodes2,int No_depend, int &input_type, int &input_id, int decide_array_type, int decide_array_id, Node *now,
                   vector<lut_arr> &array_list1, vector<sa_arr> &array_list2, vector<magic_arr> &array_list3) {
    //如果当前操作数来自寄存器，需要调用寄存器读函数
    //时间和能量加在哪里？
    //根据当前节点，获取当前的时间
    double time_n = time_only_lut(nodes2,array_list1, array_list2, array_list3, now, decide_array_type, decide_array_id);
        ...
        if (No_depend == 1) //depend1
        {
            ...
            if (input_type == 1) //来自lut的输出，写回表肯定是空的
            {
                    ...
                    && array_list1[input_id].lut_out != now->depend1->node_id)//对不上
                {
                    ...
                    int back_type_ready[3] = {-1, 3, 4};
                    int back_type = back_type_ready[rand() % 3];
                    cout << "随机选择的back_type: " << back_type << endl;
                    int back_id = 0;
                    if (back_type == -1) back_id = -1;
                    if (back_type == 3) {
                        if (array_list3.empty())
                    ...
                    write_back_lut(nodes2,1, input_id, now->depend1, array_list1, array_list2, array_list3, back_type, back_id);
                    //写回到哪里从哪里接着读取
                    //重新调用find_input
                    ...
                    data_read_lut(nodes2,1, input_type, input_id, decide_array_type, decide_array_id, now,
                                  array_list1, array_list2, array_list3);
        ...
        if (No_depend == 2) //depend2
        {
            ...
            if (input_type == 1) //来自lut的输出,写回表肯定是空的
            {
                    ...
                    && array_list1[input_id].lut_out != now->depend2->node_id)//对不上
                {
                    ...
                    write_back_lut(nodes2,1, input_id, now->depend2, array_list1, array_list2, array_list3, back_type, back_id);
                    ...
                    //修改input_type和input_id继续调用函数
                    data_read_lut(nodes2,2, input_type, input_id, decide_array_type, decide_array_id, now,
                                  array_list1, array_list2, array_list3);
...
void input_logic_lut(vector<Node> &nodes2,int operand_num, int input1_type, int input1_id, int input2_type, int input2_id,
        ...
        case 3: //MAGIC
        {
            ...
            if (operand_num == 1) {
                ...
                if (input1_type != 3 || input1_id != decide_array_id) {
                    //获取现在的时间
                    double time_n = time_only_lut(nodes2,array_list1, array_list2, array_list3, now, decide_array_type,
                                                  decide_array_id);
                    ...
                    if (now->depend1 != nullptr) {
                        ...
                        if (cap_array_lost(nodes2,3, decide_array_id, array_list1, array_list2, array_list3) == 0) {
                            ...
                            //更新节点的时间、能量
                            time_update(nodes2,0, decide_array_type, decide_array_id, time_n, now->depend1, array_list1,
                                        array_list2,
                                        array_list3);
                ...
                if (input2_type != 3 || input2_id != decide_array_id) {
                    ...
                    if (now->depend2 != nullptr) {
                        ...
                        if (cap_array_lost(nodes2,3, decide_array_id, array_list1, array_list2, array_list3) == 0) {
                            ...
                            //更新节点的时间、能量
                            time_update(nodes2,0, decide_array_type, decide_array_id, time_n, now->depend2, array_list1,
                                        array_list2,
                                        array_list3);
                            energy_update(nodes2,0, decide_array_type, decide_array_id, array_list1, array_list2, array_list3);
                        }
...
void output_logic_lut(vector<Node> &nodes2,int decide_array_type, int decide_array_id, int op_type, Node *now,
                      vector<lut_arr> &array_list1, vector<sa_arr> &array_list2, vector<magic_arr> &array_list3) {
    //执行期间对阵列的影响
    ...
    op_lut_only(nodes2,op_type, decide_array_id, now, time_n, array_list1, array_list2, array_list3);
    //需要一个函数判断是否需要调用模块
...
void write_back_lut(vector<Node> &nodes2,int from_type, int from_id, Node *now, vector<lut_arr> &array_list1, vector<sa_arr> &array_list2,
    ...
    if (back_type == -2) { //没有指定写回的位置
        switch (from_type) {
            case -1: //来自寄存器，要写到阵列中
            {
                ...
                write_back_lut(nodes2,-1, -1, now, array_list1, array_list2, array_list3, 3, pos);
            ...
            case 1: //来自lut-out,写回顺位：4 lut-latch、3 ma、0 reg
            {
                ...
                {
                    write_back_lut(nodes2,1, from_id, now, array_list1, array_list2, array_list3, 4, from_id);
                }
                if (write_type == 3) {
                    //将所有能进行写入的阵列全部列出，进行DSE
                    ready_array = ma_list;
                    if (ready_array.empty())
                        ready_array.push_back(build(nodes2,3, 0, array_list1, array_list2, array_list3));
                    int pos = ready_array[rand() % ready_array.size()];
                    write_back_lut(nodes2,1, from_id, now, array_list1, array_list2, array_list3, 3, pos);
                }
                if (write_type == 0) //写回寄存器
                {
                    write_back_lut(nodes2,1, from_id, now, array_list1, array_list2, array_list3, 0, -1);
                }
            ...
            case 3: //来自magic存储 ,写回顺位：寄存器0、其他magic 3、sa存储 2
            {
                ...
                int write_type = ready_type[rand() % ready_type.size()];
                if (write_type == 0) {
                    write_back_lut(nodes2,3, from_id, now, array_list1, array_list2, array_list3, 0, -1);
                }
                if (write_type == 3) //写回magic
                {
                    ...
                    if (ready_array.empty())
                        ready_array.push_back(build(nodes2,3, 0, array_list1, array_list2, array_list3));
                    int pos = ready_array[rand() % ready_array.size()];
                    write_back_lut(nodes2,3, from_id, now, array_list1, array_list2, array_list3, 3, pos);
                }
            ...
            case 4: //来自lut-buffer。写回顺位：寄存器0、其他magic 3
            {
                ...
                int write_type = ready_type[rand() % ready_type.size()];
                if (write_type == 0) {
                    write_back_lut(nodes2,4, from_id, now, array_list1, array_list2, array_list3, 0, -1);
                }
                if (write_type == 3) //写回magic
                {
                    ready_array = ma_list;
                    if (ready_array.empty())
                        ready_array.push_back(build(nodes2,3, 0, array_list1, array_list2, array_list3));
                    int pos = ready_array[rand() % ready_array.size()];
                    write_back_lut(nodes2,4, from_id, now, array_list1, array_list2, array_list3, 3, pos);
                }
    ...
    } else //指定写回的位置
    {
        switch (from_type) {
            case -1: //寄存器读，阵列写 3
            {
                ...
                //将读取时间更新到节点身上
                //使用读时间更新函数
                double time_n = time_only_lut(nodes2,array_list1, array_list2, array_list3, now, back_type, back_id);
                read_time_update(nodes2,-1, -1, time_n, now, array_list1, array_list2, array_list3);
//                now->end_time += reg.reg_read_time;
                ...
                // 更新阵列写时间
                time_n = time_only_lut(nodes2,array_list1, array_list2, array_list3, now, back_type, back_id);
                //更新时间,节点，阵列
                    ...
                    case 4://lut-out 1 写回lut-buffer 4
                    {
                        ...
                        if (!array_list1[back_id].lut_latch.empty() &&
                                ...
                                //找个地方写回，from_type=4,lut-buffer
                            {
                                ...
                                write_back_lut(nodes2,4, back_id, find_node_by_number(nodes2,array_list1[back_id].lut_latch.back()),
                                           array_list1, array_list2, array_list3);
                            }
                        }
                        //清空buffer
            ...
            case 3: //来自magic存储，magic读，寄存器、sa写
            {
                switch (back_type) {
                    case 0://来自magic 3 写回寄存器 0/-1
                    {
                        ...
                        //阵列读时间更新
                        double time_n = time_only_lut(nodes2,array_list1, array_list2, array_list3, now, from_type, from_id);
                        ...
                        Reg_sum.write_num_sum++;
                        //更新寄存器写时间
                        time_n = time_only_lut(nodes2,array_list1, array_list2, array_list3, now, from_type, from_id);
                        time_update(nodes2,0, -1, -1, time_n, now, array_list1, array_list2, array_list3);
                        //更新寄存器能量
                        energy_update(nodes2,0, -1, -1, array_list1, array_list2, array_list3);
                        //更新节点的结束(执行)类型和位置
                    ...
                    case 3: //来自magic 3 写回magic
                    {
                        ...
                        array_list3[from_id].read_number++;
                        //阵列读时间更新
                        double time = time_only_lut(nodes2,array_list1, array_list2, array_list3, now, from_type, from_id);
                        ...
                        array_list3[back_id].write_number++;
                        //时间
                        time = time_only_lut(nodes2,array_list1, array_list2, array_list3, now, back_type, back_id);
                        //更新时间,节点，阵列
            ...
            case 4: //来自lut-buffer，类似于寄存器，buffer读，阵列、寄存器写
            {
                ...
                //将读取时间更新到节点身上
                //使用读时间更新函数
                double time_n = time_only_lut(nodes2,array_list1, array_list2, array_list3, now);
...
void op_lut_only(vector<Node> &nodes2,int op_type, int decide_array_id, Node *now, double time_now, vector<lut_arr> &array_list1,
    ...
    if (array_list1[decide_array_id].lut_out > 0)  //节点id>=1
    {
        ...
        else //否则判断一下其写回表和出度
        {
            ...
            {
                //设置优先级，buffer只能写回本阵列
                write_back_lut(nodes2,1, find_node_by_number(nodes2,lut_out_now)->finish_id, find_node_by_number(nodes2,lut_out_now),
                           array_list1, array_list2, array_list3);
            }
        }
    ...
    array_list1[decide_array_id].lut_out=now->node_id;
    // 更新时间
    time_update(nodes2,op_type, 1, decide_array_id, time_now, now, array_list1, array_list2, array_list3);
//更新能量
    energy_update(nodes2,op_type, 1, decide_array_id, array_list1, array_list2, array_list3);
