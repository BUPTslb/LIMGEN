...
using namespace std;
vector<Nodes> nodes = {};//节点类型的向量，里面可以放下一个个节点
vector<Nodes> nodes2 = {};
...
int main() {
    ...
    ********************************/
    ...
    int num_branch = 0;
    //定义控制步
    vector<vector<Nodes>> controlstep;
    ...
    ************************************************/
    ...
    for (int i = 0; i < json_size; i++) {
        ...
        if (type_id == 1) {
            ...
            //cout<<"OP num compute"<<endl;
        }
    }

    //cout << "hash size = " << operator_num.size() << endl;
    ...
    ******************************/
    ...
    for (int i = json_size - 1; i > 0; i--) {
        ...
        Nodes *c = find_node_by_number1(i + 1);
        //c表示当前节点
//        Nodes c=nodes[id_pos[i+1]];
        ...
        if (type_id == 3)//类型为branch,存在控制依赖
        {
            ...
            Nodes *con_node = find_node_by_number1(con);//条件节点的指针
            ...
            //cout << "branch node depend" << endl << "condition node:" << con << "  size of true:" << sizeofT << endl;
            //cout << "T: ";

            for (int there = 0; there < sizeofT; there++) {
                //cout << d[i]["Statement_when_true"][there].GetInt() << "   ";
                Nodes *a = find_node_by_number1(d[i]["Statement_when_true"][there].GetInt());
//a不是空节点,且当前控制节点没有指向
                if (a != NULL && (a->control == NULL || a->control->node_id < con)) {
                    ...
                    //cout << "there is " << con << " to " << a->node_id << "control depend" << endl;
                }
        ...
        if (type_id == 4)//类型为loop
        {
            ...
            for (int there = 0; there < sizeofL; there++) {
                ...
                Nodes *a = find_node_by_number1(there_id);
            ...
            switch (type_id) {
                ...
                case 1://类型为Op，input的类型都是string
                {
                    ...
                    if (num_input == 2) {
                        ...
                        {
                            Nodes *a = find_node_by_number1(dst_id[name_in1]);//当前输入依赖的节点1
                            if (a != nullptr)
                        ...
                        {
                            Nodes *b = find_node_by_number1(dst_id[name_in2]);//当前输入依赖的节点2
                            if (b != nullptr)
                                b->out_degree++;
                    ...
                    } else //Op输入数量为1
                    {
                        ...
                        {
                            Nodes *a = find_node_by_number1(dst_id[input_name]);//指向输入依赖
                            if (a != nullptr)
                                a->out_degree++;
                ...
                case 2://类型为assign,默认只有一个输入
                {
                    ...
                    if (d[i]["Input"].IsString() && !input_depend[0]) {
                        ...
                        Nodes *a = find_node_by_number1(dst_id[input_name]);
                        c->depend1 = a;
                        if (a != nullptr)
                            a->out_degree++;
...
//    }
    ...
    for (int p = 0; p < 1; ++p) {
        ...
        vector<vector<Nodes *>> controlstep2;


        for (auto &i: controlstep) {
            vector<Nodes *> stepnow;//当前控制步，存放节点指针
...
//                    p--;
    ...
    cout << "BEST ENERGY: latency = " << best_energy[0] << "  energy = " << best_energy[1] <<" area =  "<<best_energy[2]<< endl;
    cout << "Array num = " << array_num_energy[0] << " " << array_num_energy[1] << " "
         << array_num_energy[2] << endl;
    cout << "BEST AREA: latency = " << best_area[0] << " energy = " << best_area[1] <<" area = "<<best_area[2]<< endl;
